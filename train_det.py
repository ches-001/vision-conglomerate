...
def make_dataset(data_dir: str, **kwargs) -> DetectionDataset:
    return DetectionDataset(data_dir, **kwargs)

def make_dataloader(dataset: DetectionDataset, batch_size: int, sampler: Optional[Sampler]=None, **kwargs) -> DataLoader:
    kwargs = dict(batch_size=batch_size, **kwargs)
    if "num_workers" not in kwargs:
        kwargs["num_workers"] = os.cpu_count()
    if sampler:
        kwargs["shuffle"] = False
    return DataLoader(dataset, collate_fn=DetectionDataset.collate_fn, sampler=sampler, **kwargs)

def make_model(in_channels: int, num_classes: int, config: Dict[str, Any], anchors: Dict[str, Any]) -> DetectionNetwork:
    model = DetectionNetwork(in_channels=in_channels, num_classes=num_classes, config=config, anchors=anchors)
    model.train()
    return model
...
def run(args: argparse.Namespace, config: Dict[str, Any]):
    ...
    train_dataset = make_dataset(train_path, **img_config)
    eval_dataset = make_dataset(eval_path, **img_config)
    train_sampler = None
    eval_sampler = None
    if args.use_ddp:
    ...
    model = make_model(in_channels, num_classes, model_config, new_anchors)
    loss_fn = make_loss_fn(model, class_weights, **loss_config)
    ...
    pipeline = TrainDetectionPipeline(
        model, 
        loss_fn, 
        optimizer, 
...
if __name__ == "__main__":
    ...
    SEED = 42
    CONFIG_PATH = "config/detection/config.yaml"
    ANCHORS_PATH = "config/detection/anchors.yaml"
    ...
    # For single GPU / CPU training:: train_det.py --use_ddp --lr_schedule --batch_size=32
    # For multiple GPU training:: torchrun --standalone --nproc_per_node=gpu train_det.py --use_ddp --lr_schedule --batch_size=32
    run(args, config)